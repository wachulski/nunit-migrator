...
namespace NUnit.Migrator.Tests.Helpers
{
    ...
    public abstract partial class DiagnosticVerifier
    {
        ...
        private static readonly MetadataReference SystemXmlRef = CreateMetadataRefContaining<XmlAttribute>();
        private static readonly MetadataReference NUnitFrameworkRef = CreateMetadataRefContaining<Assert>();
        ...
        private static MetadataReference CreateMetadataRefContaining<T>()
        {
            ...
            foreach (var project in projects)
            {
                var compilationWithAnalyzers = project
                    .WithCompilationOptions(((CSharpCompilationOptions) project.CompilationOptions)
                        ...
                        //.WithWarningLevel(4)
                        // DesktopAssemblyIdentityComparer.LoadFromXml may be required for non-Desktop targeting (e.g. Mono)
        ...
        private static Document[] GetDocuments(string[] sources, string language)
        {
            ...
            if (sources.Length != documents.Length)
            {
                throw new InvalidOperationException(
                    $"Amount of sources {sources.Length} did not match amount of Documents created {documents.Length}");
            }
        ...
        private static Project CreateProject(string[] sources, string language = LanguageNames.CSharp)
        {
            ...
            var solution = new AdhocWorkspace()
                .CurrentSolution
                .AddProject(projectId, TestProjectName, assemblyName, language)
                .AddMetadataReference(projectId, CorlibReference)
                .AddMetadataReference(projectId, SystemCoreReference)
                .AddMetadataReference(projectId, CSharpSymbolsReference)
                .AddMetadataReference(projectId, CodeAnalysisReference)
                .AddMetadataReference(projectId, SystemXmlRef)
                .AddMetadataReference(projectId, NUnitFrameworkRef);
