...
namespace NUnit.Migrator.Tests.Helpers
{
    ...
    public abstract partial class DiagnosticVerifier
    {
        ...
        protected void VerifyNoDiagnosticReported(string source)
        {
            ...
            params DiagnosticResult[] expectedResults)
        {
            ...
            if (expectedCount != actualCount)
            {
                ...
                Assert.Fail("Mismatch between number of diagnostics returned, " +
            ...
            for (var i = 0; i < expectedResults.Length; i++)
            {
                ...
                else
                {
                    ...
                    {
                        Assert.Fail($"Expected {expected.Locations.Length - 1} additional locations " +
                                    $"but got {additionalLocations.Length} for Diagnostic:\r\n" +
                                    $"    {FormatDiagnostics(analyzer, actual)}\r\n");
                    }
                ...
                {
                    Assert.Fail($"Expected diagnostic severity to be \"{expected.Severity}\" " +
                                $"was \"{actual.Severity}\"\r\n" +
                                "\r\n" +
                                "Diagnostic:\r\n" +
                                $"    {FormatDiagnostics(analyzer, actual)}\r\n");
                ...
                {
                    Assert.Fail($"Expected diagnostic message to be \"{expected.Message}\" " +
                                $"was \"{actual.GetMessage()}\"\r\n" +
                                "\r\n" +
                                "Diagnostic:\r\n" +
                                $"    {FormatDiagnostics(analyzer, actual)}\r\n");
                }
            ...
            Location actual, DiagnosticResultLocation expected)
        {
            ...
            Assert.IsTrue(
                ...
                $"was actually in file \"{actualSpan.Path}\"\r\n" +
                "\r\n" +
                "Diagnostic:\r\n" +
            ...
            {
                Assert.Fail($"Expected diagnostic to be on line \"{expected.Line}\" " +
            ...
            {
                Assert.Fail($"Expected diagnostic to start at column \"{expected.Column}\" " +
                            $"was actually at column \"{actualLinePosition.Character + 1}\"\r\n" +
                            "\r\n" +
                            "Diagnostic:\r\n" +
                            $"    {FormatDiagnostics(analyzer, diagnostic)}\r\n");
            }
